...
| compare_lists | ユーザーが比較対象として選択した園を管理する                             |
...
ユーザーが比較対象として選択した園を保存する。

比較対象の選択は `/mypage/favorites` で行い、`/compare` で横並び比較する。
...
比較対象の最大件数は DB 制約ではなく API 側で制御する。
...
- 比較対象はユーザーごとに DB 保存する
- お気に入りと比較対象の重複登録は DB 側の複合ユニーク制約で防止する
- お気に入り登録上限は API 側で制御する
- 比較対象の登録上限は API 側で制御する
