...
- 保育園管理者による自園情報の取得・編集はMVP対象とする
- ENKATSU運営者向けの管理画面CRUDはMVP対象外とする
...
| school_admins             | 保育園管理者と管理対象の園を紐づける                                             |
| school_inquiries          | 保護者から園への問い合わせを管理する                                             |
| school_visit_reservations | 保護者から園への見学予約希望を管理する                                           |
...
| school_admins     | 1 : 0..1 | 保育園管理者情報を保持する         |
...
| manager_name                 | varchar(100) | YES  | -             | 経営者名                   |
| contact_person               | varchar(100) | YES  | -             | 園側担当者名               |
...
保育園管理者と管理対象の園を紐づけるテーブル。

ログイン情報やパスワードは保持しない。
認証はSupabase Authで行い、`school_admins` はログイン中ユーザーがどの園を管理できるかを判定するために利用する。

| カラム名   | 型        | NULL | デフォルト | 説明              |
| ---------- | --------- | ---- | ---------- | ----------------- |
| id         | uuid      | NO   | uuid()     | 園管理者ID        |
| user_id    | uuid      | NO   | -          | users.id を参照   |
| school_id  | bigint    | NO   | -          | schools.id を参照 |
| created_at | timestamp | NO   | now()      | 作成日時          |
| updated_at | timestamp | NO   | now()      | 更新日時          |
...
### 4.9 school_inquiries

保護者から園への問い合わせを管理するテーブル。

MVPでは実データ連携は必須ではないが、後続実装で利用する。

| カラム名   | 型        | NULL | デフォルト | 説明                         |
| ---------- | --------- | ---- | ---------- | ---------------------------- |
| id         | uuid      | NO   | uuid()     | 問い合わせID                 |
| school_id  | bigint    | NO   | -          | 問い合わせ先の園ID           |
| user_id    | uuid      | NO   | -          | 問い合わせ送信者のユーザーID |
...
### 4.10 school_visit_reservations

保護者から園への見学予約希望を管理するテーブル。

MVPでは実データ連携は必須ではないが、後続実装で利用する。

| カラム名       | 型          | NULL | デフォルト | 説明                   |
| -------------- | ----------- | ---- | ---------- | ---------------------- |
| id             | uuid        | NO   | uuid()     | 見学予約ID             |
| school_id      | bigint      | NO   | -          | 見学先の園ID           |
| user_id        | uuid        | NO   | -          | 予約送信者のユーザーID |
| preferred_date | date        | NO   | -          | 見学希望日             |
| preferred_time | varchar(20) | NO   | -          | 見学希望時間           |
| created_at     | timestamp   | NO   | now()      | 送信日時               |
| updated_at     | timestamp   | NO   | now()      | 更新日時               |
...
- `user_id` は `users.id` を参照する
- `school_id` は `schools.id` を参照する
- `user_id` と `school_id` にインデックスを設定する
- 同じユーザーと園の組み合わせが重複しないように複合ユニーク制約を設定する
...
### 5.10 school_visit_reservations
...
| v0.6       | 2026/06/30 | 保育園管理者サイト対応に伴い、`school_admins`、`school_inquiries`、`school_visit_reservations` テーブルを追加。`schools` に `manager_name`、`contact_person` を追加し、保育園管理者向けDB設計を反映                                                                                                                                             |
