...
比較APIでは、比較表で表示するための表示用テキスト項目として `itemBurdenDetail` / `weekdayEvents` / `parentAssociationFrequency` を返します。

また、チャート表示で使用するため、`lifeBurdenLevel` / `timeBurdenLevel` / `itemBurdenLevel` / `weekdayEventsLevel` / `parentAssociationLevel` も返します。
...
また、2園比較・3園比較画面で園画像を表示できるように、各園データに `imageUrl` を返します。

チャート表示で使用するため、以下の負担レベル項目も返します。
...
プレミアムユーザー向けには、`user_preferences` の希望条件を参照し、園情報との一致判定結果を `matchHighlights` として返します。

一般ユーザー、または希望条件が未設定の場合、`matchHighlights` は `null` とします。
...
| 項目                                    | 型            | 内容                         |
| --------------------------------------- | ------------- | ---------------------------- |
| `data.schools`                          | array         | 比較対象の園一覧             |
| `data.schools[].id`                     | string        | 園ID                         |
| `data.schools[].name`                   | string        | 園名                         |
| `data.schools[].area`                   | string        | エリア・市区町村             |
| `data.schools[].schoolType`             | string        | 園種別                       |
| `data.schools[].imageUrl`               | string / null | 園画像URL                    |
| `data.schools[].lifeBurdenLevel`        | string        | 生活負担レベル               |
| `data.schools[].timeBurdenLevel`        | string        | 時間負担レベル               |
| `data.schools[].itemBurdenLevel`        | string        | 持ち物負担レベル             |
| `data.schools[].weekdayEventsLevel`     | string        | 平日行事の多さ               |
| `data.schools[].parentAssociationLevel` | string        | 保護者会の負担               |
| `data.schools[].lifeBurden`             | object        | 生活負担に関する比較表示項目 |
| `data.schools[].timeBurden`             | object        | 時間負担に関する比較表示項目 |
| `data.schools[].supportInfo`            | object        | サポート情報                 |
| `data.matchHighlights`                  | object / null | 希望条件との一致判定結果     |
...
| 項目                   | 型            | 内容           |
| ---------------------- | ------------- | -------------- |
| `contactBookType`      | string / null | 連絡帳の種類   |
| `absenceContactMethod` | string / null | 欠席連絡方法   |
| `lessons`              | string / null | 園内習い事     |
| `allergySupport`       | string / null | アレルギー対応 |

#### matchHighlights

`matchHighlights` は、園IDをキーにした一致判定結果を返します。

| 項目                     | 型      | 内容                                   |
| ------------------------ | ------- | -------------------------------------- |
| `mealType`               | boolean | 希望する給食・弁当区分と一致するか     |
| `itemBurdenLevel`        | boolean | 希望する持ち物負担レベルと一致するか   |
| `diaperSupport`          | boolean | 希望するおむつ対応と一致するか         |
| `futonSupport`           | boolean | 希望する布団対応と一致するか           |
| `extendedCare`           | boolean | 希望する延長保育条件と一致するか       |
| `lessons`                | boolean | 園内習い事ありの希望と一致するか       |
| `allergySupport`         | boolean | アレルギー対応ありの希望と一致するか   |
| `weekdayEventsLevel`     | boolean | 希望する平日行事負担レベルと一致するか |
| `parentAssociationLevel` | boolean | 希望する保護者会負担レベルと一致するか |
...
このAPIで保存された希望条件は、比較APIにおける `matchHighlights` 判定、およびおすすめ順ロジックで利用します。
...
- 一般ユーザーは2件まで
- プレミアムユーザーは3件まで
- プレミアムユーザーの場合、希望条件が設定されていれば `matchHighlights` に一致判定結果を返す
- 一般ユーザー、または希望条件が未設定の場合、`matchHighlights` は `null` とする
...
比較APIでは、比較表で表示するための表示用テキスト項目に加えて、園画像表示用の `imageUrl` と、チャート表示用の負担レベル項目を返します。

また、プレミアムユーザー向けに `user_preferences` の希望条件を参照し、`matchHighlights` を返します。
...
MVPでは、園画像は `schools.image_url` に保存した画像URLをAPIレスポンスの `imageUrl` として返します。

園一覧・園詳細・園比較APIでは、フロントエンドで画像表示に利用できるように `imageUrl` を含めます。

画像URLは、MVPでは固定画像パスまたは将来拡張で外部URLに差し替える想定です。
...
保育園管理者サイト追加（#147関連）に伴い、園管理者向けのAPIを `/api/v1/school-admin` 配下に追加します。

園管理者は、ENKATSU運営側が事前に作成・提供するアカウントでログインします。

ログインはSupabase Authを利用し、バックエンド独自の `POST /api/v1/school-admin/login` は作成しません。

園管理者向けAPIでは、Authorizationヘッダーの Supabase access token を検証し、`school_admins` テーブルにログイン中ユーザーの `userId` が存在する場合のみ利用を許可します。

`school_admins` に存在しないユーザーが園管理者APIへアクセスした場合は、`403 Forbidden`（`FORBIDDEN`）を返します。

園管理者は、自身に紐づく `schoolId` の園情報のみ取得・編集できます。他園の情報は取得・編集できません。

問い合わせ・見学予約の送信・一覧取得APIは、余力実装・後続実装とします。
...
ログイン中の保育園管理者情報を取得します。

ログイン直後の画面表示や、管理対象の園情報を確認するために使用します。
...
Supabase access tokenを検証し、`school_admins` に登録されたユーザーのみ利用できます。
...
| ------------- | ------ | ------------------------ |
| `id`          | string | 園管理者ID               |
| `userId`      | string | 管理者ユーザーID         |
| `schoolId`    | string | 管理対象の園ID           |
| `school`      | object | 管理対象の園情報（簡易） |
| `school.id`   | string | 園ID                     |
| `school.name` | string | 園名                     |
...
ログイン中の保育園管理者に紐づく園の詳細情報を取得します。

園詳細情報編集画面（`/admin/edit`）の初期表示に使用します。
...
| `area`          | string        | エリア・市区町村 |
| `address`       | string        | 住所             |
| `phoneNumber`   | string / null | 園の電話番号     |
| `managerName`   | string / null | 経営者名         |
| `contactPerson` | string / null | 担当者名         |
| `imageUrl`      | string / null | 園画像URL        |

園詳細API（8-2）と同様の項目（`schoolType` / `mealType` / `itemBurdenLevel` など）も、編集対象として併せて返します。
...
ログイン中の保育園管理者に紐づく園の情報を更新します。

更新対象は、ログイン中管理者の `schoolId` に紐づく園のみとし、他園の情報は更新できません。

MVPでは本格的な画像アップロード機能は実装せず、`imageUrl` の登録・更新で画像対応を行います。
...
| ------ | --------------------------------------------------------- |
| `name` | 必須                                                      |
| その他 | 任意。MVPでは厳密な入力チェックよりも保存・反映を優先する |
...
### エラー例：他園の情報を編集しようとした場合

```json
{
  "error": {
    "code": "FORBIDDEN",
    "message": "自園以外の情報は編集できません"
  }
}
```
...
## 19-4. 問い合わせ送信（余力実装・後続実装）

### 概要

ログイン済みの保護者が、指定した園へ問い合わせを送信します。

保護者名・メールアドレスはリクエストで直接送信せず、ログイン中ユーザーの情報（`users` テーブル）を参照します。

電話番号は、現状アプリ上で登録導線がないため送信・保存対象に含めません。
...
```http
POST /api/v1/schools/:schoolId/inquiries
```
...
## 19-5. 問い合わせ一覧取得（余力実装・後続実装）

### 概要

ログイン中の保育園管理者に紐づく園宛の問い合わせ一覧を取得します。

管理者ホーム画面（`/admin`）のお問い合わせ一覧タブで使用します。

MVPでは、本APIを利用した実データ連携までは必須とせず、画面側はモックデータ表示でもよいものとします。

保育園管理者は、他園宛の問い合わせを閲覧できません。
...
| 項目                | 型     | 内容             |
| ------------------- | ------ | ---------------- |
| `data[].id`         | string | 問い合わせID     |
| `data[].message`    | string | お問い合わせ内容 |
| `data[].createdAt`  | string | 送信日時         |
| `data[].user.id`    | string | 送信者ユーザーID |
| `data[].user.name`  | string | 保護者名         |
...
## 19-6. 見学予約送信（余力実装・後続実装）

### 概要

ログイン済みの保護者が、指定した園へ見学予約希望を送信します。

保護者名・メールアドレスはリクエストで直接送信せず、ログイン中ユーザーの情報（`users` テーブル）を参照します。

電話番号は、現状アプリ上で登録導線がないため送信・保存対象に含めません。

MVPでは、予約枠の重複チェック・過去日チェックまでは必須としません。
...
```http
POST /api/v1/schools/:schoolId/visit-reservations
```
...
```json
{
  "data": {
    ...
    "schoolId": "school_1",
    "userId": "user_1",
    "preferredDate": "2026-07-30T00:00:00.000Z",
    "preferredTime": "10:30",
    "createdAt": "2026-06-29T15:00:00.000Z"
  }
}
...
## 19-7. 見学予約一覧取得（余力実装・後続実装）

### 概要

ログイン中の保育園管理者に紐づく園宛の見学予約一覧を取得します。

管理者ホーム画面（`/admin`）の見学予約一覧タブで使用します。

MVPでは、本APIを利用した実データ連携までは必須とせず、画面側はモックデータ表示でもよいものとします。

保育園管理者は、他園宛の見学予約を閲覧できません。
...
| 項目                   | 型     | 内容             |
| ---------------------- | ------ | ---------------- |
| `data[].id`            | string | 見学予約ID       |
| `data[].preferredDate` | string | 見学希望日       |
| `data[].preferredTime` | string | 見学希望時間     |
| `data[].createdAt`     | string | 送信日時         |
| `data[].user.id`       | string | 送信者ユーザーID |
| `data[].user.name`     | string | 保護者名         |
| `data[].user.email`    | string | メールアドレス   |
...
## 19-8. バリデーション方針（園管理者関連）

- 園管理者ログインは、Supabase Authのログイン処理に準拠する
- フロント上の表示は「アカウントID」とするが、内部的にはSupabase Auth上のemailとして扱う
- バックエンド側では独自のログインバリデーションは行わない
- 園管理者API利用時、`school_admins` に登録されているユーザーかどうかを確認し、存在しない場合は `FORBIDDEN` を返す
- 園情報編集（`PATCH /school-admin/school`）は、園名のみ必須とし、その他項目は任意とする
- 問い合わせ送信の `message` は必須・空文字不可とし、最大文字数を設定する
- 見学予約送信の `preferredDate` / `preferredTime` は必須とする

---

## 19-9. セキュリティ・注意点（園管理者関連）

- 保育園管理者は自園の情報のみ取得・更新できる
- 保育園管理者は他園の情報を取得・編集できない
- `school_admins` に存在しないユーザーは園管理者APIを利用できない
- 問い合わせ・見学予約の実データ連携を行う場合、保育園管理者は他園宛の問い合わせ・見学予約を閲覧できない
- 問い合わせ内容はHTMLとして描画しない
- 保護者の電話番号は、保護者側で登録導線がないため扱わない
- 保護者名・メールアドレスは、問い合わせ・見学予約テーブルに直接保存せず、`users` テーブルを参照する
- パスワードはアプリ側DBには保存せず、Supabase Authで管理する

---

## 19-10. MVPでは対応しない範囲（園管理者関連）
...
| v0.6       | 2026/06/29 | 園一覧・園詳細・園比較APIの `imageUrl` を明記。比較APIにチャート表示用の負担レベル項目を追加し、`matchHighlights` の現在仕様に合わせて説明とレスポンス例を修正                                                                                                                                                                                                                                                                                                             |
| v0.7       | 2026/06/30 | 保育園管理者サイト追加（#147関連）に伴い、「19. 園管理者API」を新設。`GET /school-admin/me`・`GET /school-admin/school`・`PATCH /school-admin/school` を必須実装APIとして追加し、`POST /schools/:schoolId/inquiries`・`GET /school-admin/inquiries`・`POST /schools/:schoolId/visit-reservations`・`GET /school-admin/visit-reservations` を余力実装・後続実装APIとして追加。認証は既存方針と同様にSupabase Authを利用し、`school_admins` テーブルで権限判定する方針を明記 |
